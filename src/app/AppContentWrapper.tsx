...
const screenConfig: Record<
  string,
  { component: React.ComponentType<any>; contentComponent?: React.ComponentType<any> }
> = {
  ...
  PrivacyPolicy: { component: PrivacyPolicyPage },
  TermsOfService: { component: TermsOfServicePage },
...
export function AppContentWrapper() {
  ...
  const navigate = useCallback((screen: ScreenKey, props?: Record<string, any>) => {
    ...
    setNavigationState((prevState) => {
      if (mainTabs.includes(screen)) {
        return { ...prevState, activeTab: screen };
      }
  ...
  if (showSplashAd) return <SplashScreenAd />;
  ...
  return (
    <main className="h-screen w-screen bg-background flex flex-col">
      ...
      {mainTabs.includes(activeStack[activeStack.length - 1]?.screen) && (
        <BottomNav
