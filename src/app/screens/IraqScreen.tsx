...
export function IraqScreen({ navigate, goBack, canGoBack }: ScreenProps) {
  ...
  useEffect(() => {
    ...
    const tryFinishLoading = () => {
      if (
        (!favoritesUnsubscribe || favoritesInitialized) &&
        (!pinnedMatchesUnsubscribe || pinnedInitialized)
      ) {
        setIsLoading(false);
      }
    };
    ...
    if (db) {
      ...
      pinnedMatchesUnsubscribe = onSnapshot(
        q,
        (snapshot) => {
        ...
        <div className="px-4 pt-4">
          ...
          {isAdmin && pinnedMatches.filter((m) => m.isEnabled).length === 0 && (
            <Button
              ...
              onClick={() => navigate("ManagePinnedMatch", { matchId: null })}
            >
        ...
        >
          ...
          <TabsContent value="our-league" className="flex-1 overflow-auto">
            {isLoading ? (
              ...
              <OurLeagueTab navigate={navigate} leagueId={ourLeagueId} />
            )}
          </TabsContent>
